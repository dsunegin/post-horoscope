...
const main = async (): Promise<string> => {
    try {
        ...
        let start = new Date(2021,5,1);
        let end = new Date(2021,5,5);
        ...
        while (start < end) {
            ...
            const dateLoc = format(start, 'do MMMM yyyy', {locale: Locale});
            const dateLocTomorrow = format(tomorrow, 'do MMMM yyyy', {locale: Locale});
            console.log(dateLoc);
            ...
            for (let i = 0, itpl: any; (itpl = zodiacArr[i]); ++i) {
                ...
                let hrefTomorrow = assets.aliasSlug(`${Zodiac[i].name} - гороскоп на сегодня ${dateLocTomorrow}`, false);
                let hrefTextTommorrow = `<a href="${subURL}${hrefTomorrow}" title="${Zodiac[i].name} - гороскоп на завтра"><h2>${Zodiac[i].name} - гороскоп на завтра<h2></a>`
                let PostText = nunjucks.renderString(itpl.post_tpl, zodiacArr);
                PostText = `<img alt="${PostTitle}" src="${PostImg}"> <p>${PostText}</p> ${hrefTextAdd} ${hrefTextTommorrow}`;
